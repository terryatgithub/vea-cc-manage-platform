<template>
    <ContentCard :title="title" @go-back="$emit('go-back')">
        ...
        <div class="base-tit">
            <!-- <img src="../../../themes/images/base.png"> -->
            <span>基本信息</span>
        </div>
        <el-form ref="blockForm" :rules="rules" :model="block" label-width="140px">
            <el-form-item label="系统功能名称" prop="pluginInfo.pluginName" :rules="rules.pluginInfo.pluginName">
            ...
            <template  v-if="block.pluginInfo.pluginParentType === 'builtIn'">
                <el-form-item  label="固定刷新时间"  prop="pluginInfo.refreshTime">
                    <div class="el-input" style="max-width: 400px">
                        <ccTimeSpinner v-model="block.pluginInfo.refreshTime" >
                        </ccTimeSpinner>
                    </div>
                </el-form-item>
            ...
            <el-form-item  v-show="block.pluginInfo.pluginParentType !== 'builtIn'"  label="类型"  prop="pluginInfo.pluginParentType" 
                :rules="rules.pluginInfo.pluginParentType">
                    <el-select  class="plugin-parent-type-selector"  
                    :value="block.pluginInfo.pluginParentType"  @input="handleChangePluginParentType" :disabled="mode === 'edit'" placeholder="请选择">
                        <el-option  v-for="item in pluginParentTypes" 
                        :key="item.id" :label="item.label"  :value="item.value">
                        </el-option>
                </el-select>
            </el-form-item>
            <el-form-item  v-if="pluginParentType !== 'secKill'"  label="内容源"   prop="pluginInfo.source" :rules="rules.pluginInfo.source">
                <el-radio-group  :value="block.pluginInfo.source"   @input="handleChangeSource"  :disabled="isDisabledSource">
                    <el-radio v-for="(item, key) in SOURCE" :key="key" :label="item.value" >{{ item.label }}</el-radio>
                </el-radio-group>
            </el-form-item>
            <!--类型位标记推荐位-->
            <template v-if="pluginParentType === 'sign'">
                <el-form-item label="标记推荐位类型" prop="pluginInfo.pluginType" :rules="rules.pluginInfo.pluginType">
                    <el-input v-model.trim="block.pluginInfo.pluginType"></el-input>
                </el-form-item>
            </template>
            <!--end-->
            <el-form-item  v-if="pluginParentType !== 'sign' && pluginTypes.length > 0"  label="多功能推荐位类型"  prop="pluginInfo.pluginType"
                :rules="rules.pluginInfo.pluginType">
                <el-select  class="plugin-type-selector" :value="block.pluginInfo.pluginType" @input="handleChangePluginType" placeholder="请选择" style="width: 300px; max-width: 300px">
                    <el-option  v-for="item in pluginTypes" :key="item.id"  :label="item.label"  :value="item.value">
                    </el-option>
                </el-select>
            </el-form-item>
            ...
            <template v-if="pluginParentType !== 'sign'">
                <div  v-for="(item, index) in block.rlsInfo" :key="index">
                    ...
                    <!--builtIn-->
                    <template v-if="block.pluginInfo.pluginParentType === 'builtIn'">
                        <el-form-item label="状态栏文字" :prop="'rlsInfo.' + index + '.title'" :rules="rules.barText">
                    ...
                    <template v-if="versionHasTitle">
                        ...
                        <el-form-item label="副标题" :prop="'rlsInfo.' + index + '.subTitle'" :rules="rules.subTitle">
                            <el-input v-model.trim="item.subTitle"></el-input>
                        </el-form-item>
                    </template>
                    <!--builtIn-->
                    <template v-if="block.pluginInfo.pluginParentType === 'secKill'">
                        <el-form-item label="商品ID" :prop="'rlsInfo.' + index + '.goodsId'" :rules="rules.goodsId">
                            <el-input v-model.trim="item.goodsId"></el-input>
                        </el-form-item>
                        <el-form-item label="商品名称" :prop="'rlsInfo.' + index + '.title'" :rules="rules.goodsId">
                            <el-input v-model.trim="item.title"></el-input>
                        </el-form-item>
                    </template>
                    <el-form-item label="海报" :prop="'rlsInfo.' + index + '.poster.pictureUrl'" :rules="rules.poster.pictureUrl">
                        <div class="poster" @click="handleSelectPosterStart(index)">
                            <img class="poster__img" v-if="item.poster.pictureUrl" :src="item.poster.pictureUrl" />
                        </div>
                    </el-form-item>
                    <template v-if=" pluginType === 'REFERENCE_ACTIVITY'">
                        <el-form-item  label="异形焦点" :prop="'rlsInfo.' + index + '.extendInfo.focusImgUrl'" :rules="rules.focusImgUrl">
                            <el-button  type="primary"  @click="handleSelectFocusImgStart(index)" style="margin-bottom: 10px">
                            选择异形焦点
                            </el-button>
                            <div v-if="item.extendInfo.focusImgUrl" class="focus-transition">
                                <i title="移除异形焦点" class="focus-transition__remove el-icon-circle-close" @click="handleRemoveFocusImg(index)"></i>
                                <img class="focus-transition__img" :src="item.extendInfo.focusImgUrl"/>
                            </div>
                        </el-form-item>
                        <el-form-item  label="存活时间" :prop="'rlsInfo.' + index + '.extendInfo.aliveTime'" :rules="rules.aliveTime">
                            <div class="el-input" style="max-width: 400px">
                                <ccTimeSpinner v-model.number="item.extendInfo.aliveTime" :options="{ min: '00:05',height: 34}" >
                                </ccTimeSpinner>
                            </div>
                        </el-form-item>
                        <el-form-item label="点击次数" :prop="'rlsInfo.' + index + '.extendInfo.clickCount'" :rules="rules.clickCount">
                            <el-input v-model.number="item.extendInfo.clickCount"></el-input>
                        </el-form-item>
                    </template>
                    <el-form-item v-if="pluginParentType === 'multi' || pluginParentType === 'secKill' || pluginParentType === 'builtIn'" 
                    label="打开方式" :rules="rules.openMode">
                        <el-select :value="item.openMode" @input="handleChangeOpenMode(item, $event)">
                            <el-option  v-if="pluginParentType === 'builtIn' ||  pluginType === 'REFERENCE_MOVIE_VIP' || pluginType === 'REFERENCE_ACTIVITY'" 
                             label="网页" value="webpage"></el-option>
                            <template v-if=" pluginType === 'REFERENCE_ACTIVITY'">
                        ...
                        <el-button v-if="item.openMode === 'app' " type="primary"  @click="handleSelectClickStart(index)">
                        快速填充
                        </el-button>
                    </el-form-item>
                    <template v-if="item.openMode === 'webpage'">
                        <el-form-item label="网页地址" :prop="'rlsInfo.' + index + '.onclick.webpageUrl'" :rules="rules.webpageUrl">
                        ...
                        <el-form-item label="应用版本号" :prop="'rlsInfo.' + index + '.onclick.webpageAppVersion'" :rules="rules.webpageAppVersion">
                            <el-input v-model.trim="item.onclick.webpageAppVersion"></el-input>
                        </el-form-item>
                    </template>
                    <template v-if="item.openMode === 'video'">
                        <el-form-item label="视频名称" :prop="'rlsInfo.' + index + '.onclick.videoName'" :rules="rules.videoName">
                            <el-input v-model.trim="item.onclick.videoName"></el-input>
                        </el-form-item>
                        <el-form-item label="视频地址" :prop="'rlsInfo.' + index + '.onclick.videoUrl'" :rules="rules.videoUrl">
                            <el-input v-model.trim="item.onclick.videoUrl"></el-input>
                        </el-form-item>
                    </template>
                    <template v-if="item.openMode === 'tab'">
                        <el-form-item label="版面" :prop="'rlsInfo.' + index + '.onclick.tab'" :rules="rules.tab">
                    ...
                    <template v-if="item.openMode === 'picture'">
                        <el-form-item label="选择图片" :prop="'rlsInfo.' + index + '.onclick.picture'" :rules="rules.picture">
                            <el-upload :action="urls.uploadImg" :on-success="createUploadSuccessHandler(index)" :on-remove="createUploadRemoveHandler(index)"
                                :multiple="false" :file-list="item.onclick.picture"
                                accept="image/png,image/gif,image/jpeg,image/bmp" list-type="picture">
                                <el-button type="primary">点击上传</el-button>
                                <span slot="tip" class="el-upload__tip"> 提示:只能上传png/gif/jpg/bmp文件</span>
                            </el-upload>
                        </el-form-item>
                    </template>
                    <ccAppParamsForm v-if="item.openMode === 'app'" v-model="item.onclick" label-width="140px" :prop-prefix="'rlsInfo.' + index + '.onclick.'"/>
                </div>
            </template>
        </el-form>
    ...
    props: {
                ...
                picture: [
                    { 
                        ...
                        validator: function(_, value, cb) {
                            cb(value && value.length > 0 ? undefined : new Error('请上传图片'))
                        } 
                    },
                    {
                        required: true,
      ...
      computed: {
        ...
        baseHasTitle() {
            ...
            const pluginParentType = this.pluginParentType
            const pluginType = this.pluginType
            return pluginParentType !== 'secKill' && pluginParentType !== 'builtIn' && (pluginType && pluginType !== 'REFERENCE_MOVIE_VIP')
        }
    },
    watch: {
    },
      methods: {
        ...
        setPluginType(val) {
            ...
            this.getPluginVersions(val)
                .then(function(versions) {
                    ...
                    helper.title = ''
                    helper.subTitle = ''
                    block.rlsInfo = versions.map(function(item) {
        ...
        // handleSelectPosterCancel() {
        //     this.showPostSelector = false
        //     this.selectingPostForIndex = undefined
        // },
        // handleSelectPosterEnd(val) {
        //     const index = this.selectingPostForIndex
        //     const item = this.block.rlsInfo[index]
        //     item.poster.pictureId = val.pictureId
        //     item.poster.pictureUrl = val.pictureUrl
        //     this.selectingPostForIndex = undefined
        //     this.showPostSelector = false
        // },
        ...
        // },
        // handleSelectClickEnd(val) {
        //     val = JSON.parse(val.onlickJson)
        //     const index = this.selectingClickForIndex
        //     const item = this.block.rlsInfo[index]
        //     item.onclick = this.parseOnclick(val)
        //     this.showClickSelector = false
        //     this.selectingClickForIndex = undefined
        // },
        ...
        // handleSelectTabCancel() {
        //     this.showTabSelector = false
        //     this.selectingTabForIndex = undefined
        // },
        // handleSelectTabEnd(tab) {
        //     const index = this.selectingTabForIndex
        //     const item = this.block.rlsInfo[index]
        ...
        // handleSelectFocusImgCancel() {
        //     this.showFocusImgSelector = false
        //     this.selectingFocusImgForIndex = undefined
        // },
        // handleSelectFocusImgEnd(focusImg) {
        //     const index = this.selectingFocusImgForIndex
        //     const item = this.block.rlsInfo[index]
        //     this.$set(item.extendInfo, 'focusImgUrl', focusImg.pictureUrl)
        //     this.selectingFocusImgForIndex = undefined
        //     this.showFocusImgSelector = false
        // },
        ...
        validateData(data, cb) {
            this.$refs.blockForm.validate(function(valid) {
                if (valid) {
                    cb(data)
                } else {
                    this.$message({
        ...
        setData(data) {
            ...
            block.rlsInfo.forEach(function(item) {
                ...
                if (item.onclick) {
                    ...
                    const paramsIndexed = originOnclick.params.reduce(function(result, item) {
                    ...
                    switch(item.openMode) {
                        case 'webpage':
                            item.onclick = {
                                webpageType: originOnclick.byvalue === 'coocaa.intent.action.browser' ? '1' : '2',
            ...
            if (pluginParentType !== 'secKill' && (pluginType && pluginType !== 'REFERENCE_MOVIE_VIP')) {
        ...
        parseData(data) {
            ...
            data.rlsInfo.forEach(function(item) {
                ...
                switch(item.openMode) {
                    case 'webpage':
                        if (originOnclick.webpageType === "1") {  //浮窗网页
                            item.onclick = {
                        ...
                        } else if (originOnclick.webpageType === "2") {    //全屏网页
                            item.onclick = {
        ...
        submit(formData) {
            this.validateData(formData, function() {
        ...
        handleChangeActivityFlag(val) {
            ...
            if (val) {
                this.block.rlsInfo.unshift(this.genRlsInfo({
                    label: '活动形式', 
                    dataType: 4
                }))
            } else {
