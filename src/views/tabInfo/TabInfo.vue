...
export default {
    ...
    methods: {
        ...
        handleToggleAllPanel(isCollapse) {
            this.tabInfo.pannelList.forEach(function(item) {
        ...
        handleChangeTabCategory(tabCategory) {
            ...
            if (shouldClearContents) {
                this.$confirm("是否要切换业务分类，从不分源分类切换到分源分类之后所选择的版块或二级版面数据将会清除掉?", "提示", {
        ...
        handlePreviewPanel(panel) {
            ...
            const url = this.basicFn.numToPannelTypeUrl(row.pannelType) + '/preview.html?id=' + row.pannelGroupId + '&version=' + version;
            this.FastDevTool.createDialogWin('edit-view', {
                ...
                content: this.FastDevTool.createIframe(url),
                onClose: function() {
                    this.updatePanelVersion(row, function() {
        ...
        handleClickBlock(data) {
            ...
            switch(true) {
                case isJiangSu && panelData.pannelGroupId.toString().indexOf(10) === 0:
                    // 江苏泰州的不能编辑酷开(10)这边的
                    url = this.basicFn.numToPannelTypeUrl(panelData.pannelType) + '/preview.html?id=' + panelData.pannelGroupId + '&version=' + version;
                    break;
                case panelStatus ===  STATUS.draft :
                    // 如果是草稿，直接修改
                    url = this.basicFn.numToPannelTypeUrl(panelData.pannelType) + '/edit.html?id=' + panelData.pannelGroupId + '&version=' + version;
                    url += "&clickActiveIndex=" + activeIndex + "&clickBlockIndex=" + blockIndex
                    break;
                case panelStatus === STATUS.accepted:
                   // 创建副本
                    url = this.basicFn.numToPannelTypeUrl(panelData.pannelType) + '/editHistory.html?id=' + panelData.pannelGroupId + '&version=' + version;
                    url += "&clickActiveIndex=" + activeIndex + "&clickBlockIndex=" + blockIndex
                    break;
            ...
            this.FastDevTool.createDialogWin('edit-view', {
                ...
                content: this.FastDevTool.createIframe(url),
                onClose: function() {
                    this.updatePanelVersion(panelData, function() {
        ...
        handleChangeSource(val) {
            this.$confirm("是否要切换内容源，切换内容源之后所选择的版块或二级版面数据将会清除掉！", "提示", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
                type: "warning"
            })
        ...
        loadPanelDetail(panel, checkDuplicateVersion) {
            ...
            this.$service.panelGetDetail({id, version}).then((panelDetail) => {
              ...
              const panelList = originPanelList.map(function(item) {
                  return {
                      panelTitle: item.pannelTitle,
                      blocks: panelDetail.parentType === 'function' ? [] : this.getPanelBlocks(item) 
                  }
              }.bind(this))
              delete panelDetail.currentVersion
              delete panelDetail.duplicateVersion
              this.panelListIndexed[panel.pannelGroupId] = Object.assign({}, panel, panelDetail, {
                  panelList: panelList,
        ...
        handleReOrderPanel(dragInfo, dropInfo) {
            ...
            switch(true) {
                case dragType === 'SPEC' && dropType === 'NORMAL': {
                    ...
                    // 插入
                    const targetIndex = dragIndex <= dropIndex 
                        ? dropIndex
                        : dropIndex + 1
                ...
                case dragType === 'CROWD' && dropType === 'NORMAL': {
                    ...
                    const panelItem = {
                        type: 'SPEC',
                        crowdPanels: [
                            dragPanelItem.crowdPanels[dragCrowdIndex]
                        ]
                    }
                    // 从旧的地方删除
                    if (dragPanelItem.crowdPanels.length === 1) {
                        ...
                        panelList.splice(dragIndex <= dropIndex ? Math.max(dropIndex, 0) : (dropIndex + 1), 0 ,panelItem)
                    } else {
                        this.doRemovePanel(dragIndex, dragCrowdIndex)
                ...
                case dragType === 'CROWD' && dropType === 'CROWD': {
                    ...
                    if (dragIndex === dropIndex) {
                        ...
                        dropCrowdPanels.splice(dragCrowdIndex <= dropCrowdIndex 
                            ? dropCrowdIndex 
                            : (dropCrowdIndex + 1)
                        , 0, panelItem)
                        ...
                        while(--length >= 0) {
                            if (dropCrowdPanels[length].dmpInfo.crowdId === panelItem.dmpInfo.crowdId) {
                                break;
                            }
                        }
                        if (length >= 0) {
                ...
                case dragType === 'NORMAL' && dropType === 'NORMAL':  {
                    ...
                    this.doRemovePanel(dragIndex)
                    // 插入
                    panelList.splice(dragIndex <= dropIndex 
                        ? dropIndex 
                        : (dropIndex + 1)
                    , 0, dragPanelItem)
                    break;
                ...
                case dragType === 'NORMAL' && dropType === 'CROWD': {
                    ...
                    this.selectCrowdCallback = function(dmpInfo) {
                        ...
                        // 插入
                        panelList[dropIndex].crowdPanels.splice(dropCrowdIndex + 1, 0, dragPanelItem.panel)
                        // 删除原来的
                        this.doRemovePanel(dragIndex)
                        this.updateDuplicates()
                    }
        ...
        getPanelBlocks(panel) {
          ...
          const layoutHeight = lastBlock.y + (lastBlockResize.height || lastBlock.height)
          ...
          const calculateFactory = function() {
            ...
            if (type === "Expander" && originBlockCount === 2) {
                ...
                const x = layoutWidth + (n - 2) * space +  (n - 3) * targetBlock.width;
          ...
          const blocksWithResources = blocks.map(function(item, index) {
            ...
            resource.contentPosition = Object.assign({}, item, {resize: undefined})
            ...
            panelList.forEach(function(item, index) {
                if (item.type === 'NORMAL') {
                    ...
                    crowdPanels.forEach(function(cItem, index) {
                        ...
                        // 判断跟普通的是否重复
                        specPanelIndexed[pannelGroupId] = specPanelIndexed[pannelGroupId] || []
        ...
        handleChangePanelOrderEnd(index) {
            if (this.currentPanelIndex === index) {
                ...
                this.tabInfo.pannelList = [].concat(panelList.slice(0, newIndex), item, panelList.slice(newIndex))
        ...
        handleShowTimeShelf() {
            ...
            this.validateFormData(data, function() {
          ...
          if (tabStatus === STATUS.draft) {
            ...
            this.$refs.tabForm.validate(function(valid) {
              if (!valid) {
                ...
                if(data.tabType == 1 && data.hasSubTab === 1) {
                    ...
                    for(let i = 0, length = panelList.length; i < length; i++) {
                        ...
                        if (item.type === 'NORMAL') {
                            ...
                            const crowdPanels = item.crowdPanels
                            for(let j = 0, lengthJ = crowdPanels.length; j < lengthJ; j++) {
                ...
                if (data.tabType === 2 && data.alumbTabLongBg && data.pannelList.length > 6) {
                    //  专题版面且含有长图
                    error = '长图专题版块不能超过6个'
                }
            ...
            let panelInfoList = []
            {
                ...
                for(let i = 0, length = panelList.length; i < length; i++) {
                   ...
                   if (item.type === 'NORMAL') {
                       ...
                       panelInfoItem.dmpPanelList = item.crowdPanels.map(function(p, pIndex) {
                           return {
                                pannelGroupId: p.id,
        ...
        setTabInfo(data) {
            ...
            if (data.panelInfoList) {
                const panelToLoad = []
                pannelList = data.panelInfoList.map(function(item) {
                    if (item.isDmpPanel) {
                        return {
                            type: 'SPEC',
                            isCollapse: false,
                            isShowAll: false,
                            crowdPanels: item.dmpPanelList.map(function(pItem) {
