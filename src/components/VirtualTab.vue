        ...
        <template v-for="(panelItem, index) in panels">
            ...
            >
                <div class="panel-group-title">
                    ...
                    />
                    <i v-if="!readOnly"  title="删除组" @click="handleRemovePanel(index)" class="tab-placeholder__group-remove el-icon-close"></i>
                </div>
                <div class="panel-group-content">
                    <div v-if="panelItem.type === 'SPEC'">
                        ...
                        <template 
                            v-for="(item, crowdIndex) in panelItem.crowdPanels"
                         >
                            <cc-var 
                            ...
                            >
                                    <template v-if="!readOnly" scope="{panel: panel}">
                                        <i title="删除" @click="handleRemovePanel(index, crowdIndex)" class="tab-placeholder__panel-remove el-icon-close"></i>
                                        <a 
                                        ...
                                        <div class="tab-plcacholder-info">
                                            ...
                                            >
                                                {{ panel.panelId }}
                                                <span class="panel-status--waiting" v-show="panel.status !== 4">({{ STATUS_TEXT[panel.status] }})</span>
                                            </div>
                                            <div
                                                class="tab-placeholder-info__link"
                                                @click="handleOpenPanel(panel)"
                        ...
                        >
                            <a @click="handleShowAllPanels(index)" title="显示所有板块">
                                <i class="el-icon-arrow-down"></i>共 {{ panelItem.crowdPanels.length }} 个板块
                            </a>
                        </div>
                    </div>
                    <div 
                        v-else 
                        class="tab-placeholder__normal-panel"
                    >
                        <cc-var 
                            :class="{
                                'tab-placeholder__panel-wrapper': true, 
                        ...
                        >
                                <template scope="{panel: panel}">
                                    <i v-show="!readOnly" title="删除" @click="handleRemovePanel(index)" class="tab-placeholder__panel-remove el-icon-close"></i>
                                    ...
                                    <div class="tab-plcacholder-info">
                                        <div 
                                            ...
                                            @click="handleOpenPanel(panel)"
                                        >
                                            {{ panel.panelId }}<span class="panel-status--waiting" v-show="panel.status !== 4">({{ STATUS_TEXT[panel.status] }})</span>
                                        </div>
                                        <div
                                            class="tab-placeholder-info__link"
                                            @click="handleOpenPanel(panel)"
